...
/*
 * Copyright 2012 2013 Darko Pantić (pdarko@myopera.com)
 *
...
(function (window, document, extension, widget) {
	...
	function insertNotice(event) {
		...
		if (id)
			id.value += ", ExtendTube " + widget.preferences.version + ", " + window.navigator.platform
	}
	...
	function connectionLost(event) {
		...
		notice = document.querySelector("#xtt-notice-style")
		if (notice)
			notice.parentNode.removeChild(notice)
	}
	...
	function addStyle() {
		...
		style.setAttribute("id", "xtt-notice-style")
		style.textContent = "\
.reports #feedback-list .comment p {\n\
	max-height: 300px;\n\
	overflow: auto;\n\
}\n\
#xtt-notice {\n\
...
})(window, window.document, opera.extension, widget)
