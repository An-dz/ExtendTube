 ...
 *
 * @copyright 2010 2011 2012 2013 <a href="mailto:pdarko@myopera.com">Darko Pantić</a>
...
self.yt.__defineSetter__("playerConfig", function(value) {
	...
	if (playerRoot)
		playerRoot.dataset.swfConfig = window.JSON.stringify(yt.playerConfig)
})
...
self.ytplayer.__defineGetter__("config", function() {
	return playerConfig
})
self.ytplayer.__defineSetter__("config", function(value) {
	yt.playerConfig = value
})
...
if (window.ytplayer) {
	for (var property in window.ytplayer)
		ytplayer[property] = window.ytplayer[property]

	window.ytplayer = ytplayer
}
else {
	window.__defineGetter__("ytplayer", function () {
		return ytplayer
	})
	window.__defineSetter__("ytplayer", function (value) {})
}
...
function addExternalScript() {
	if (!document.querySelector("head"))
		return setTimeout(addExternalScript, 50)
...
function addManualSearch() {
	...
	references.lyricsSearchLog.querySelector(".ext-close-results").classList.add("ext-hidden")
...
function addPreviewButton(element) {
	...
	var addto = element.querySelector(".addto-button, .addto-container, .ext-addto"),
		button = createButton({
			"class": getButtonClass().replace("yt-uix-tooltip-reverse", " yt-uix-button-default addto-button video-actions ext-thumb-preview"),
	...
	if (addto.nodeName.toLowerCase() != "button")
		...
		button.style.setProperty("right", window.getComputedStyle(addto).getPropertyValue("width"))
	}
}
...
function addRatingMeter(thumb, rating) {
	...
	if (score || score == 0)
		tooltip = xtt.ll.getString("RATING_LIKE")
					  .replace("%pcent", score.toString().substr(0, 4))
					  + " (" + rating.numLikes + " / " + rating.numDislikes + ")"
	else {
		meter.classList.add("ext-rating-unavailable")
		score = 50
	}
...
/**
 * Event listener for <cite>BeforeExternalScript</cite> event.
...
function beforeExternalScript(event) {
	...
	if (/googlesyndication|doubleclick|googletagservices|googleadservices|\/uds\/api\/ads\/|\/jsapi\?autoload[=\w\[\{:"]+"name":"ads"/.test(decodeURIComponent(src))) {
		if (preferences.hidepageads) {
			...
			log.warn('Linked script blocked. URI: ' + src + '.')
		}
		else
...
function beforeScript(event) {
	...
	if (/opera\.xtt/.test(event.element.text))
		log.warn('Linked script modified. Script URI: ' + src + '.')
	else
		log.info('Linked script not modified. URI: ' + src + '.')
}
...
function collapseLoopButton() {
	...
	references.loopButton.classList.remove("yt-uix-button-active")
...
function collapseLyricsContainer() {
	...
	references.lyricsFooter.querySelector("button").textContent =
		xtt.ll.getString("LYRICS_BUTTON_SHOW")

	// Scroll page for video to be visible.
	if (preferences.scrollonlyricsdisplay) {
		var headline = document.querySelector("#watch7-container, #watch-container, #watch-headline")
...
function controlPlayer(command, data) {
	...
	try {
		...
		if (controlAttempt.count < 10) {
			...
			log.error('An attempt to issue “' + command + '” command to player failed',
					  controlAttempt.count,
					  'times in the past ' + ((Date.now() - controlAttempt.start) / 1000),
					  'seconds.',
...
function decodeStreamMap(map) {
	map = map.split(",").map(function (url) {
		...
		if (!/signature%3D/.test(url)) {
			log.info('Signature not found in URI.')
...
function disableThumbPreview(id) {
	var preview = document.querySelector("[data-ext-preview-id=\"" + id + "\"].ext-preview-enabled")
	if (!preview)
		return
...
function domContentLoaded(event) {
	...
	// Move extension added style to the bottom of head element.
	Array.prototype.forEach.call(document.querySelectorAll("style[id^=\"ext-\"]"), function (style) {
	...
	// Add class for hiding language alert.
	Array.prototype.forEach.call(document.querySelectorAll("#alerts .yt-alert"), function (langalert) {
...
function expandLoopButton() {
	...
	references.loopButton.classList.add("yt-uix-button-active")
...
function expandLyricsContainer() {
	...
	references.lyricsFooter.querySelector("button").textContent =
		xtt.ll.getString("LYRICS_BUTTON_HIDE")
...
function extractVideoInfo(response) {
	...
	var xml = parser.parseFromString(response.xml, "text/xml")
	...
	if (xml.documentElement.nodeName != "feed") {
		...
		log.info('Got response to video ratings query.',
				 'Response length is ' + response.xml.length + '.',
				 'Response time ' + (Date.now() - response.timeStamp) + 'ms.')
...
function getButtonClass() {
	return "yt-uix-button yt-uix-tooltip yt-uix-tooltip-reverse ext-button"
}
...
function getKeyName(event) {
	...
	if (event.which) {
		if (keyCode1.hasOwnProperty(event.keyCode.toString()))
			keyStr = keyCode1[event.keyCode.toString()]
	}
	else {
		if (keyCode0.hasOwnProperty(event.keyCode.toString()))
			keyStr = keyCode0[event.keyCode.toString()]
	}

	if (event.location == 3 && event.key != "NumLock")
		return "Np " + keyStr
	else
		return keyStr
}
	...
	if (path[1]) {
		if (path[0] == "cleanwatch") {
			...
			if (path[1] == "allsidebar") {
				...
				style = preferences.style[path[0]][path[1]]
			...
			return preferences.style[path[0]][path[1]]
					   .replace(/@color@/g, preferences[path[0]][path[1]])
		else
			return preferences.style[path[0]][path[1]]
	}
		...
		return preferences.style.popout
				   .replace(/@width\s/g, size.width)
				   .replace(/@height\s/g, size.height + 30)
				   .replace(/@topmargin\s/g, topmargin)
	}
	else
...
function getVideoID(anchor) {
	...
	if (anchor.nodeName.toLowerCase() == "a") {
		if (/^\/watch/.test(anchor.pathname)) {
			vid = anchor.search.match(/v=([^&]+)/)
			if (vid && vid[1])
				id = vid[1]
		}
		else if (/^\/user\//.test(anchor.pathname)) {
			vid = anchor.hash.match(/^#p\/l\/([^\/]+)/)
			if (vid && vid[1])
				id = vid[1]
		}
	}
	...
	if (!id) {
		vid = anchor.querySelector(".video-thumb img")
		if (vid) {
			if (vid.dataset.thumb)
				vid = vid.dataset.thumb
			else
...
function handleAutoPlay() {
	...
	if (xtt.video.isEmbedded) {
		if  (xtt.video.isPopup && xtt.player.autoplay && xtt.player.state != 1) {
			log.info('Video will be played. Autoplay: ' + xtt.player.autoplay
					+ '; player state: ' + xtt.player.state + '.')
		...
		log.info('Video won\'t be started. Autoplay is: false.')
}
...
function injectForceLoopCheck(text) {
	// Hide "Next video starts N seconds" message
	text = text.replace(/("NEAR_END"==)/, "opera.xtt&&!opera.xtt.player.isLoopForced()&&$1")
	...
	text = text.replace(stateChange, "$1if(a==0&&opera.xtt&&opera.xtt.player.isLoopForced())return;")
...
function lyricsSearch(event) {
	...
	references.lyricsSearchLog.querySelector(".ext-close-results").classList.remove("ext-hidden")
...
function makeApi() {
	...
	xtt.ll = /** @lends xtt.ll */ {
		...
		setLanguage: function setLanguage(lang) {
			if (preferences.overridelocale || !lang)
				...
				if (preferences.localisedStrings.hasOwnProperty(lang))
					language = lang
				else if (/^[a-z]{2,3}_[A-Z]{2}$/.test(lang)
						 && preferences.localisedStrings.hasOwnProperty(lang.split("_")[0]))
	...
	xtt.player = /** @lends xtt.player */ {
		...
		playcount: 0,
		/**
		 * Reference to YouTube Player API.
		 *
			...
			switch (command) {
				...
				case "seek":
					...
					if (typeof data == "string") {
						...
						if (/^[+\-]/.test(data))
							data = this.playerElement.getCurrentTime() +
									   xtt.video.length * parseInt(data, 10) / 100
						else
		 ...
		 */
		enableWide: function enableWidePlayer(wide, howwide) {
			var container = document.querySelector("#watch7-container, #watch-container")
			if (!container || xtt.video.isEmbedded)
				return

			if (wide !== undefined) {
				updateWidePlayerStyle()

				// Use YouTube API.
				if (typeof window.ytPlayerSIZE_CLICKEDplayer1 == "function")
					window.ytPlayerSIZE_CLICKEDplayer1(wide)
				// Fallback.
				else {
					if (wide) {
						container.classList.add("watch-wide")
						container.classList.add("watch-playlist-collapsed")

						if (howwide === undefined)
							howwide = preferences.howwide

						if (howwide == "large") {
							container.classList.remove("watch-medium")
							container.classList.add("watch-large")
						}
						else {
							container.classList.remove("watch-large")
							container.classList.add("watch-medium")
						}

						log.info('Player size is changed to wide size.')
					}
					else {
						container.classList.remove("watch-wide")
						container.classList.remove("watch-large")
						container.classList.remove("watch-medium")
						container.classList.remove("watch-playlist-collapsed")

						log.info('Player size is changed to normal size.')
					}
				}
			}
			// Toggle wide.
			else
				enableWidePlayer(!container.classList.contains("watch-wide"))
		},
		/**
		...
		playerReady: function playerReady() {
			var _this = xtt.player

			// Remember reference to player.
			_this.playerElement =
				document.querySelector("embed[flashvars], #movie_player-html5,"
									  + " #video-player-html5, .player-container #video-player,"
			...
			if (xtt.video.ishtml5 && !_this.api)
				_this.api = _this.playerElement

			if (_this.api)
					_this.api.addEventListener("onStateChange", xtt.player.stateChanged)
			else
				log.error('Failed to get reference to YouTube player API')
		...
		stateChanged: function playerStateChanged(newstate) {
			...
			if (newstate == 1 && !xtt.video.isPreview) {
				...
				if (!_this.ready) {
					log.warn('Video playback started before player is ready.',
							 'It will be paused.')
					...
					log.warn('Video playback started but autoplay option is unknown.',
							 'It will be paused.')

					_this.control("pause")
					return
				}
				else if (!_this.autoplay && _this.playcount < 1 && xtt.video.ishtml5) {
					log.warn('Video playback started but autoplay option is false.',
							 'It will be paused.')
				...
				switch (newstate) {
					...
					case 1:
						log.info('Video playback started.',
								 '(ready: ' + _this.ready + ', autoplay: ' + _this.autoplay
									 + ', playcount: ' + _this.playcount + ').')
		...
		writePlayer: function () {
			...
			if (xtt.video.isEmbedded)
				...
				if (preferences.extensionwriteplayer)
					...
					setTimeout(function () {
						yt.player.embed.call(window, player.getAttribute("id"), yt.playerConfig)
					}, 100)
			}
		}
	}
	...
	xtt.shortcut = /** @lends xtt.shortcut */ {
		...
		disable: function disableShortcuts() {
			...
			document.removeEventListener("keydown", shortcutAction, false)
	...
	xtt.ui.add = /** @lends xtt.ui.add */ {
		...
		downloadMenuItem: function addDownloadMenuItem(item, before, internal) {
			...
			if (internal)
				menuItem.dataset.extInternal = "true"
		...
		extraButtons: function addExtraButtons(node) {
			...
			var buttoncnt = document.createElement("span"),
				// Frame-step button.
				fstep = createButton({
					"class": getButtonClass() + " ext-button-middle yt-uix-button-text",
					"id": "ext-framestep-button",
					"data-tooltip-text": xtt.ll.getString("STEP_FRAME_FORWARD")
				}),
				// Seek forward button.
				fseek = createButton({
					"class": getButtonClass() + " ext-button-end yt-uix-button-text",
					"id": "ext-seekforward-button"
				}),
				// Seek back button.
				bseek = createButton({
					"class": getButtonClass() + " ext-button-start yt-uix-button-text",
		...
		lyricsContainer: function addLyricsContainer(where, node) {
			...
			header.insertAdjacentHTML("afterbegin",
									  "<h4>" + xtt.ll.getString("LYRICS_LYRICS") + "</h4>")

			// Search log container.
			var infoel = document.createElement("p")
			infoel.setAttribute("class", "ext-lyrics-body-info")
			// Hide button for search log.
			infoel.insertAdjacentHTML("afterbegin", "<span class=\"ext-close-results\">✘</span>")
			...
			var button = createButton({ "class": "metadata-inline yt-uix-button yt-uix-button-text" })
			...
			// Show/hide lyrics.
			expand.addEventListener("click", xtt.ui.apply.toggleLyrics, false)
			...
			spliter.setAttribute("class", "ext-spliter")
			spliter.appendChild(document.createElement("hr"))
			spliter.firstChild.classList.add("yt-horizontal-rule")
			...
			if (node.dataset.extLyricsPosition)
				where = node.dataset.extLyricsPosition
		...
		thumbPreview: function addThumbnailPreview() {
			...
			Array.prototype.forEach.call(thumb, function (anchor) {
				...
				anchor.dataset.extPreviewId = vid
			})
		...
		videoRatings: function addVideoRatings() {
			...
			partition.forEach(function (list) {
				...
				sendMessage("load external resource", {
					uri: "http://gdata.youtube.com/feeds/api/videos/batch?fields="
							 + "entry(batch:status,media:group/yt:videoid,gd:rating,yt:rating)",
					method: "post",
					postdata: postdata,
					header: header,
					about: "ratings",
				...
				log.info('Request for video information has been made.',
						 'There are ' + list.length + ' videos in request.',
						 'Request length is ' + postdata.length + '.')
			})
		}
	}
	...
	xtt.ui.apply = /** @lends xtt.ui.apply */ {
		...
		cleanPage: function cleanPage(data) {
			...
			if (!data.featured && data.suggestions) {
				var feat = document.querySelector("#watch7-sidebar .ext-featured-videos,"
												 + " #watch-sidebar .ext-featured-videos")
				if (!feat || window.getComputedStyle(feat).getPropertyValue("display") == "none")
		 ...
		 * <a href="/lib/preferences.js">preferences.js</a>.
		 */
		customColors: function customColors(customcolor) {
			var style = getStyle("customcolorextra"),
	...
	xtt.ui.remove = /** @lends xtt.ui.remove */ {
		...
		thumbPreview: function removeThumbnailPreview(removePreview) {
			...
			var loadMore = document.querySelectorAll("#watch-more-from-user, #watch-more-related,"
	...
	xtt.video = /** @lends xtt.video */ {
		...
		changeConfig: function changeConfig(data) {
			...
			if (data && typeof data == "object") {
				if (typeof data != "object") {
					log.warn('An error occurred while trying to hide in-video ads.',
				...
				if (!this.isEmbedded) {
					...
					if (preferences.disableAutoPlay && preferences.disableautobuffering)
						data.autoplay = 0
					else
						data.autoplay = 1
				}
		...
		getVideoSize: function () {
			...
			if (xtt.player.state == -1) {
				...
				if (size.hasOwnProperty(quality))
				...
				quality = xtt.player.playerElement.getPlaybackQuality()
		...
		error: function () {
			...
			if (preferences.loglevel > 0)
				console.error(log.source + "\n" + "["
							  + formatTime(log.buffer[0].time,
										   log.buffer[log.buffer.length - 1].time)
							  + "] " + mesg + "\n")
		},
		/**
 ...
 *
 * @see https://developers.google.com/youtube/2.0/developers_guide_protocol_batch_processing
 *
 * @param {Array} list
 * List of video IDs.
 */
...
function manualSearch(event) {
	...
	is = parseInt(is.getPropertyValue("border-right-width")) * 2
			 + parseInt(is.getPropertyValue("padding-right")) * 2
	bs = parseInt(bs.getPropertyValue("padding-right")) * 2
			 + parseInt(bs.getPropertyValue("width"))

	input.style.setProperty("width", (parseInt(fs.width) - bs + is) + "px")
}
	...
	switch (event.data.subject) {
		...
		case "close self":
			if (window == window.parent) {
				if (parseFloat(opera.version()) < 12)
					window.close()
				else
	...
	switch (event.data.subject) {
		...
		case "external resource loaded":
			if (event.data.data.about == "lyrics")
				...
				updateRatings(extractVideoInfo(event.data.data))
			break
...
function modifyDocument(skipLanguage) {
	...
	container.classList.add("ext-actions-container")
	...
	if (preferences.prefbutton)
		xtt.ui.add.preferencesButton(buttons)
	if (preferences.enabledownload)
		xtt.ui.add.downloadButton(buttons)
	if (preferences.loopbutton)
		xtt.ui.add.loopButtons(buttons)
	...
	if (preferences.lyrics)
		xtt.ui.add.lyricsContainer("beforeend",
								   document.querySelector(".ext-lyrics-container") || container)
...
function parseLyrics(data) {
	...
	if (lyricsFragment) {
		var junk = lyricsFragment.querySelectorAll(".rtMatcher, i, p"),
			internalLinks = lyricsFragment.querySelectorAll("a:not([href^=\"http\"])"),
			licensed = false
		...
		Array.prototype.forEach.call(internalLinks, function (link) {
			link.setAttribute("href", "http://lyrics.wikia.com" + link.getAttribute("href"))
		})
...
function parseSearchResults(data) {
	...
	if (lyrics === null || lyrics.textContent.trim() == "Not found") {
		status = "continue"
		info = xtt.ll.getString("LYRICS_SEARCH_NO_RESULT")
				   .replace("%song", current.artist + " - " + current.title)
		fireEvent()
		...
		log.info('Bingo! Lyrics are found for artist “' + current.artist + '” and title “'
					 + current.title + '”. Trying to download them.')
	...
	switch (data.key) {
		case "enabledownload":
			if (data.data[data.key])
				xtt.ui.add.downloadButton(document.querySelector(".ext-actions-right"))
			else
		...
		case "prefbutton":
			if (data.data[data.key])
				xtt.ui.add.preferencesButton(document.querySelector(".ext-actions-right"))
			else
				xtt.ui.remove.preferencesButton()
			break
		case "loopbutton":
			if (data.data[data.key])
				xtt.ui.add.loopButtons(document.querySelector(".ext-actions-right"))
			else
				xtt.ui.remove.loopButtons()
			break
		case "extrabuttons":
			if (data.data[data.key])
				xtt.ui.add.extraButtons(document.querySelector(".ext-actions-right"))
			else
		...
		case "lyrics":
			if (data.data[data.key])
				xtt.ui.add.lyricsContainer("beforeend",
										   document.querySelector(".ext-lyrics-container")
											   || document.querySelector(".ext-actions-container"))
			else
		...
		case "overridehistory":
			if (event.detail.newValue)
				...
				window.history.navigationMode = "automatic"
			break
		case "popoutrealsize":
...
function searchLyrics(song) {
	...
	var uri = "http://lyrics.wikia.com/api.php?artist=" + encodeURIComponent(song.artist)
				  + "&song=" + encodeURIComponent(song.title) + "&fmt=xml"
...
function shortcutAction(event) {
	...
	if (/input|textarea/i.test(event.target.toString()) && /^(Shift\+)?[^+]+$/.test(shortcut))
		return
	else if (/button/i.test(event.target.toString())
				 && /^((Ctrl\+)?Shift\+)?(Space|Enter)/.test(shortcut))
		return
	...
	switch (shortcut) {
		...
		case preferences.shortcut.popplayer:
			if (document.querySelector("embed[flashvars], #movie_player-html5, #video-player-html5,"
									  + " .player-container #video-player"))
				xtt.ui.apply.popoutPlayer(!document.body.classList.contains("ext-popout-player"))
			else
				preventDefaultAction = false
			break
		default:
			if (preferences.exposeplayershortcuts && xtt.player.playerElement) {
				switch (shortcut) {
					case "1":
					case "2":
					case "3":
...
function translateKeyEvent(event) {
	// Not DOM 3
	if (event.key === undefined && event.type == "keydown")
		return
	// DOM 3
	if (event.key !== undefined && event.type == "keypress")
		return
	...
	if (event.key === undefined)
		keyCombination.push(getKeyName(event))
	else {
		if (!event.key || event.key.charCodeAt(0) < 32 || event.key.charCodeAt(0) == 127)
			keyCombination.push(getKeyName(event))
		else if (!/Alt|Control|Shift/.test(event.key)) {
			if (event.location == 3 && event.key != "NumLock")
				keyCombination.push("Np " + event.key)
			else
...
function updateDownloadMenu() {
	...
	if (xtt.video.urlmap.length) {
		...
		xtt.video.urlmap.forEach(function (item) {
			...
			if (videoFormat.hasOwnProperty(item.itag)) {
...
function updateLoopButton() {
	...
	var tooltip = document.querySelector("body > div[id*=\"tooltip\"] div[id*=\"content\"]"),
		test = xtt.ll.getStringsArray("LOOP_ENABLE", "LOOP_DISABLE", "LOOP_SECTION_DISABLE",
									  "LOOP_SECTION_ENABLE").join("")

	if (xtt.player.loopSection.enabled) {
		references.loopButton.classList.add("ext-loop-on")
		references.loopButton.classList.add("ext-loop-force")
		references.loopButton.dataset.tooltipText = xtt.ll.getString("LOOP_SECTION_DISABLE")
		...
		if (ctrlDown) {
			references.loopButton.dataset.tooltipText = xtt.ll.getString("LOOP_DISABLE")
			if (tooltip && test.indexOf(tooltip.textContent) > -1)
				tooltip.textContent = xtt.ll.getString("LOOP_DISABLE")
		}
		else {
			references.loopButton.dataset.tooltipText = xtt.ll.getString("LOOP_SECTION_ENABLE")
			if (tooltip && test.indexOf(tooltip.textContent) > -1)
		...
		if (ctrlDown) {
			references.loopButton.dataset.tooltipText = xtt.ll.getString("LOOP_ENABLE")
			if (tooltip && test.indexOf(tooltip.textContent) > -1)
...
function updateSeekButtons(time) {
	...
	references.seekForwardButton.dataset.tooltipText =
		xtt.ll.getString("SEEK_FORWARD") + " (+" + time + "s)"
	references.seekBackButton.dataset.tooltipText =
		xtt.ll.getString("SEEK_BACK") + " (-" + time + "s)"
}
...
function updateThumbPreviewButton(id, state) {
	var anchor = document.querySelector("[data-ext-preview-id=\"" + id + "\"].ext-preview-enabled"),
		thumb
	...
	if (anchor) {
		...
		thumb = anchor.querySelector(".ext-thumb-preview")
	}

	if (thumb) {
		if (state == 1) {
			thumb.classList.add("ext-pause")
			thumb.dataset.tooltipText = xtt.ll.getString("PREVIEW_END")
		}
		else {
			thumb.classList.remove("ext-pause")
			thumb.dataset.tooltipText = xtt.ll.getString("PREVIEW_START")
		}
	}
}
...
function updateThumbPreviewTime(id, time) {
	var vtime = document.querySelector("[data-ext-preview-id=\"" + id
									  + "\"].ext-preview-enabled .video-time")
	if (!vtime)
		return

	if (!vtime.dataset.extVideoLength)
		vtime.dataset.extVideoLength = vtime.textContent

	if (typeof time == "number")
		vtime.textContent = time.toTimeString(true)
	else
		vtime.textContent = vtime.dataset.extVideoLength
}

function updateWidePlayerStyle() {
	var wideStyle = document.querySelector("head > #ext-wide-player-style")
	if (wideStyle && preferences.howwide == "auto")
		return removeElement(wideStyle)

	// Update existing style.
	if (wideStyle)
		wideStyle.textContent = getStyle("wideplayer/" + preferences.howwide)
	// Create new style.
	else {
		wideStyle = document.createElement("style")
		wideStyle.setAttribute("type", "text/css")
		wideStyle.setAttribute("id", "ext-wide-player-style")
		wideStyle.appendChild(document.createTextNode(getStyle("wideplayer/" + preferences.howwide)))
		document.querySelector("head").appendChild(wideStyle)

		log.info('Wide player style is enabled.')
	}
}
...
function writeFlashPlayer(playerRootElement) {
	if (!playerRootElement)
		return

	var embed = document.createElement("embed"),
		flashvars = [], value

	embed.setAttribute("type", "x-shockwave-flash")
	embed.setAttribute("src", yt.playerConfig.url)

	for (var attr in yt.playerConfig.attrs)
		embed.setAttribute(attr, yt.playerConfig.attrs[attr])
	for (var param in yt.playerConfig.params)
		embed.setAttribute(param, yt.playerConfig.params[param])

	for (var arg in yt.playerConfig.args) {
		if (yt.playerConfig.args[arg] === null)
			value = "None"
		else if (yt.playerConfig.args[arg] === true)
			value = "True"
		else if (yt.playerConfig.args[arg] === false)
			value = "False"
		else
			value = yt.playerConfig.args[arg].toString()

		flashvars.push(arg + "=" + encodeURIComponent(value))
	}

	embed.setAttribute("flashvars", flashvars.join("&"))
	playerRootElement.appendChild(embed)
}
...
function writeLyrics(data) {
	...
	if (data && data.lyrics) {
		var str = xtt.ll.getString("LYRICS_LYRICS_FOR")
					  .replace("%song", "<strong>" + data.artist + " - " + data.title + "</strong>")
