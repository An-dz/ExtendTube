/*
 * Copyright 2009 2010 2011 2012 2013 Darko Pantić (pdarko@myopera.com)
 *
...
function fillPreferences() {
	var control = document.querySelectorAll("fieldset:not(.dont-listen) input[id], "
											+ "fieldset:not(.dont-listen) select[id]")
		...
		if (group[1]) {
			...
			if (element.type == "text" || element.type == "number")
				...
				if (prefItem[group[1]].substr(0, 1) == "#") {
					...
					var alpha = prefItem[group[1]].match(/(\d?(\.\d+)?)\)$/)
					if (alpha && alpha[1])
						alpha = alpha[1]
					else
						alpha = 1
...
function resetPreferences(event) {
	...
	if (!confirm.checked)
		window.alert("To confirm that you understand repercussions of this action please"
					 + " check check-box on the left side of this button.")
	else {
...
function importPreferences(event) {
	...
	imported = imported.value.substring(imported.value.indexOf("{"))
...
function displayStorageInfo() {
	alert("Widget storage information:\n\n"
		  + "Used space: " + storageGetUsedSpace() + " characters.\n"
		  + "Available space: " + storageGetAvailableSpace() + " characters.")
}

function storageGetUsedSpace() {
	return Object.keys(pref).reduce(function (length, key) {
			if (key == "length")
				return length
			return length + key.length + pref.getItem(key).length
		}, 0)
}

function storageGetAvailableSpace() {
	var testString = "\u0000",
		testKeys = [],
		N = 0,
		storageLength = 0

	while (2 * testString.length < 25e4)
		testString += testString

	testString = testString + testString.substr(0, 25e4 - testString.length)

	while (testString.length) {
		// If tested length is greater than 10 million assume that storage
		// quota is unlimited. If we do not do this check and User Agent
		// have no quota limit, this method will never stop filling storage
		// untill User Agent crashes or system runs out of disk space.
		if (storageLength > 1e7)
			break

		try {
			pref.setItem("QuotaTest" + N, testString)
			testKeys.push("QuotaTest" + N)
			storageLength += testString.length + ("QuotaTest" + N++).length
		}
		catch (exception) {
			if (exception.code == DOMException.QUOTA_EXCEEDED_ERR)
				testString = testString.substr(Math.floor(testString.length / 2) + 1)
			else
				// There's still available space but some other error occurred.
				break
		}
	}
