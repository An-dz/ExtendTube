...
function messageReceived(message, sender) {
	if (message.subject == "here is message log") {
...
function broadcastRequest() {
	chrome.runtime.sendMessage({ subject: "give me message log"})
}
...
function printLog(data) {
	if (data.id) {
		...
		clearTimeout(logs[data.id].oldLogTimeout)
	}
	...
	data.buffer.forEach(function (mesg) {
		var time = mesg.msgc.toPaddedString(3, " ") + " ["
				   + formatTime(data.buffer[0].time, mesg.time, atime) + "]",
...
function updateList(message, sender) {
	...
	if (typeof logs[data.id] != "object") {
		...
		logs[data.id].source = message.ports[0]
		logs[data.id].oldLogTimeout = NaN

		tabs.appendChild(new Option(data.title, data.id))
	}
	else
		logs[data.id].source = sender.tab.id
}
...
function requestLog(from) {
	...
	try {
		chrome.tabs.sendMessage(logs[from].source, { subject: "are you still there" })
		logs[from].oldLogTimeout = setTimeout(function () {
			printOldLog(from)
		}, 2e3)
	}
	catch (error) {
...
function printOldLog(from) {
	showInfo("Message log cannot be fetched from page: “" + logs[from].title
			 + "”. Page is probably closed. Old log will be displayed instead.", true)
...
// ╰──────────────────────────────────────────────────╯\n\
//\n\
// Exported debug messages\n\
//\n\
// ExtendTube: " + extVersion + "\n\
// User-Agent: " + navigator.appVersion + "\n\
...
function pageScrolled(event) {
	...
	if (document.documentElement.scrollTop > 164) {
		ctrl.parentNode.style.setProperty("height",
										  getComputedStyle(ctrl.parentNode).getPropertyValue("height"))
		ctrl.classList.add("fixed")
	}
	else {
		ctrl.classList.remove("fixed")
		ctrl.parentNode.style.removeProperty("height")
	}
}
