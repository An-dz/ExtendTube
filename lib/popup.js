...
function DOMLoaded(event) {
	var xhr = new XMLHttpRequest(),
		customColours = widget.preferences.getPref("enablecustomcolors"),
		coloursInPopUp = widget.preferences.getPref("customcolorsinpopup")
	...
	if (customColours && coloursInPopUp) {
		...
		style.textContent = xhr.responseText
							   .replace(/@pbgColor/g, color.pageBackgroundPrimary)
							   .replace(/@pColor/g, color.pagePrimary)
							   .replace(/@chColor/g, color.pageBackgroundSecondary)
							   .replace(/@lColor/g, color.link)
...
function addVideo(data) {
	...
	if (data.id.indexOf("&html5") > -1)
		title.insertAdjacentHTML("afterbegin",
								 "<img src=\"../image/html5.png\" title=\"HTML5 video\">")
	if (data.id.indexOf("&embedded") > -1) {
		if (data.id.indexOf("&preview") > -1)
			title.insertAdjacentHTML("afterbegin",
									 "<img src=\"../image/preview.png\" title=\"Embedded video preview\">")
		else
			title.insertAdjacentHTML("afterbegin",
									 "<img src=\"../image/iframe.png\" title=\"IFrame embedded video\">")
	...
	if (this.className.indexOf("link") < 0) {
		...
		bgProcess.extension.tabs.create({
			url: "http://www.youtube.com/watch?v="
				 ...
				 + "#t=" + this.parentNode.querySelector("meter").value,
			focused: true
		})
...
function updateTooltip(event) {
	var hover = this.dataset.length * event.x
					/ window.parseInt(window.getComputedStyle(this, null).width)
	this.nextElementSibling.textContent = hover.toTimeString(true)
	this.nextElementSibling.style.left = event.x + "px"
}

function seekVideo(event) {
	var position = this.dataset.length * event.x
					   / window.parseInt(window.getComputedStyle(this, null).width)
	video[this.parentNode.parentNode.id].postMessage({
		subject: "playback control",
		data: {
	...
	log.info('pop-up:',
			 '“seek video” (to ' + position + ') request sent to video '
