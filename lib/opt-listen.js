...
function addListeners() {
	var control = document.querySelectorAll("fieldset:not(.dont-listen) input[id], fieldset:not(.dont-listen) select[id]"),
		...
		colorRecommend = document.getElementById("customcolor-recommend"),
	...
	customStyleSave.addEventListener("click", function (event) {
		document.getElementById("customstyle").value =
			document.getElementById("customstyle").value
					.replace(/\r/g, "")
					.replace(/\s*(\B-(moz|webkit|ms)-|\b(filter|zoom):)[^;\{\}\n]+/g, "")
					.replace(/\s*[\w\-]+:(;|\n|\})/g, "$1")
					.replace(/;{2,}/g, ";")
					.replace(/\s*\n/g, "\n")
	...
	// Load recommended custom colours.
	colorRecommend.addEventListener("click", function (event) {
		pref.setPref("customcolor", recommendedPreferences.customcolor)
	}, false)
	...
	storageInfo.addEventListener("click", displayStorageInfo, false)
