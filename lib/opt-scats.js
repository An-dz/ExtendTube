...
function changeShortcut(event) {
	...
	document.addEventListener("keydown", catchKeyCombination, false)
}
...
function saveCapture(event) {
	...
	document.removeEventListener("keydwon", catchKeyCombination, false)
	hideCaptureDialog()
...
function abortCapture(event) {
	hideCaptureDialog()
	document.removeEventListener("keypress", catchKeyCombination, false)
	document.removeEventListener("keydown", catchKeyCombination, false)
}
...
function catchKeyCombination(event) {
	...
	if (event.ctrlKey)
		keyCombination.push(makeKey("Control"))
	if (event.metaKey)
	...
	if (event.key === undefined)
		...
		if (!event.key || event.key.charCodeAt(0) < 32 || event.key.charCodeAt(0) == 127)
			...
			if (event.location == 3 && event.key != "NumLock")
				keyCombination.push(makeKey("Np " + event.key))
			else
				keyCombination.push(makeKey(event.key))
		}
	}
