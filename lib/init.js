...
var bgProcess = chrome.extension.getBackgroundPage(),
...
var log = {
		...
		lsmesg: function (atime, buffer) {
			...
			return buffer.map(function (message) {
					return '[' + formatTime(buffer[0].time, message.time, atime)
						   + "] " + message.type + ' ' + message.mesg
				}, this).join("\n")
		},
		src: "ExtendTube\n‾‾‾‾‾‾‾‾‾‾",
		info: function () {
...
window.setInterval(function () {
	...
	if (targetLength < log.buffer.length - 1) {
		log.info('Performing log buffer maintenance.',
				 'Log buffer have", log.buffer.length, "messages')
		targetLength = log.buffer.length - targetLength + 2
...
window.addEventListener("load", function () {
	Array.prototype.forEach.call(document.querySelectorAll("a[href^=\"mailto:\"]"), function (mail) {
